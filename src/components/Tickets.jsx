...
const Tickets = () => {
  return (
    <>
    <h3 id="ticket" className="text-xl p-2 mt-3">Tickets</h3>
    <div className="w-[800px] bg-[#f6f6fc] p-10 ml-20">
      {  Jobs["ticktes"].map((ticket, index) =>
          <div key={`TICKET-${index}`} className="shadow-slate-700 rounded-lg mt-3 flex flex-col p-2 gap-3 bg-white">
            <h3>{ticket.title}</h3>
            <p>{ticket.desc}</p>
